  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="container mx-auto py-8 px-4 max-w-6xl">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">Ranked Results</h1>
        ...
        {isLoading ? (
          ...
          <Card className="p-12">
            <div className="flex flex-col items-center justify-center text-center">
              ...
              <Button onClick={() => window.location.href = "/rank-resumes"} data-testid="button-go-to-rank">
                Go to Rank Resumes
              </Button>
            </div>
          </Card>
          ...
          <div className="space-y-6">
            {results.map((cv, index) => {
              ...
              return (
                <Card key={cv.id} data-testid={`card-result-${cv.id}`}>
                  <CardHeader>
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <Badge variant="secondary" className="font-mono" data-testid={`badge-rank-${cv.id}`}>
                            #{index + 1}
                          </Badge>
                          <CardTitle className="text-xl" data-testid={`text-candidate-name-${cv.id}`}>
                            {cv.candidateName}
                          </CardTitle>
                        </div>
                        <p className="text-sm text-muted-foreground mb-3" data-testid={`text-candidate-email-${cv.id}`}>
                          {cv.candidateEmail}
                        </p>
                          ...
                          {strengths.length > 0 ? (
                            <ul className="space-y-2" data-testid={`list-strengths-${cv.id}`}>
                              {strengths.map((strength, idx) => (
                                <li
                                  key={idx}
                                  className="text-sm text-muted-foreground pl-4 border-l-2 border-green-600/50 dark:border-green-400/50"
                                >
                                  {strength}
                                </li>
                              ))}
                          ...
                          {weaknesses.length > 0 ? (
                            <ul className="space-y-2" data-testid={`list-weaknesses-${cv.id}`}>
                              {weaknesses.map((weakness, idx) => (
                                <li
                                  key={idx}
                                  className="text-sm text-muted-foreground pl-4 border-l-2 border-red-600/50 dark:border-red-400/50"
                                >
                                  {weakness}
                                </li>
                              ))}
      ...
      <Dialog open={!!selectedCV} onOpenChange={() => setSelectedCV(null)}>
        <DialogContent className="max-w-4xl max-h-[80vh]">
          ...
          <ScrollArea className="h-[60vh] w-full rounded-md border p-4">
            <div className="whitespace-pre-wrap text-sm font-mono" data-testid="text-cv-content">
